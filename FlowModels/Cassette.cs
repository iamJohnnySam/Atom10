...
public class Cassette : INotifyPropertyChanged
{
    ...
    public Cassette()
    {
        ...
        if (Slots.Count != Capacity)
            throw new ErrorResponse(ErrorCode.ProgramError, $"Cassette capacity {Capacity} does not match number of slots {Slots.Count}.");

        if (IsMovableCassette && SlotMoveTime == 0)
            throw new ErrorResponse(ErrorCode.ProgramError, $"Time not set for Movable Cassette.");
    }
    ...
    {
        if (slot < 1)
            throw new ErrorResponse(ErrorCode.SlotsNotEmpty, $"Cassette has No Available Slots");

        else if (slot > Capacity)
            throw new ErrorResponse(ErrorCode.SlotOutOfBounds, $"Cassette has only {Capacity} slots. Slot {slot} is out of bounds.");
    }
    private void PayloadInterlock(Payload payload)
    {
        if (PayloadType != payload.PayloadType)
            throw new ErrorResponse(ErrorCode.PayloadTypeMismatch, $"Cassette accepts {PayloadType} and not {payload.PayloadType}.");
    }



    protected internal List<Reservation> SetReservation(ReservationType reservationType, int startingSlot, int slotCount, List<Payload>? payloads = null)
    {
        if (slotCount <= 0)
            throw new ErrorResponse(ErrorCode.SlotOutOfBounds, "Slot Count cannot be 0 or less.");

        if (payloads != null && payloads.Count != 0 && payloads.Count != slotCount)
            throw new ErrorResponse(ErrorCode.MissingArguments, "Slot Count does not match the payloads passed in");
    ...
    protected internal void MoveToSlot(int slot)
    {
        if (!IsMovableCassette)
            throw new ErrorResponse(ErrorCode.CassetteNotMovable, "Cassette is not movable");
