...
public class Layout : INotifyPropertyChanged
{
    ...
    public LayoutState State
    {
        ...
        {
            state = value;
            TransactionLogger.Instance.Info(new TransactionLog($"Simulator State has changed to {value}."));
            OnPropertyChanged();
        }
    }
    ...
    public void AddProcess(ProcessStructure processStruct)
    {
        if (Processes.ContainsKey(processStruct.ProcessId))
            throw new ErrorResponse(ErrorCode.ProgramError, $"Process with ID {processStruct.ProcessId} already exists.");
    ...
    public void AddStation(StationStructure stationStruct)
    {
        for (int i = 0; i < stationStruct.Count; i++)
        {
            ...
            foreach (int processId in stationStruct.ProcessIds)
            {
                if (!Processes.ContainsKey(processId))
                    throw new ErrorResponse(ErrorCode.ProgramError, $"Process with ID {processId} does not exist.");
    ...
    public void AddManipulator(ManipulatorStructure manipulatorStruct)
    {
        for (int i = 0; i < manipulatorStruct.Count; i++)
        {
            ...
            if (manipulatorStruct.EndEffectors.Count != manipulatorStruct.EndEffectorSlots.Count)
                throw new ErrorResponse(ErrorCode.MissingArguments, "EndEffectors and EndEffectorSlots count must be the same.");
